...
export default function ActionInput({
  ...
  const getPlaceholder = () => {
    switch (step) {
      case 'buy':
        return `Max: ${maxValue} bushels available`;
      case 'feed':
        return `Max: ${maxValue} bushels available`;
      case 'plant':
        return `Max: ${maxValue} acres available`;
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (step === 'buy') {
      ...
      if (bushelsNeeded > constraints?.maxBushels!) {
        setError(`Not enough grain for seed. Need: ${bushelsNeeded}, Available: ${constraints?.maxBushels}`);
        return;
      }
      const peopleNeeded = Math.ceil(numValue / 10);
      if (peopleNeeded > constraints?.peopleAvailable!) {
        setError(`Not enough people. Need: ${peopleNeeded}, Available: ${constraints?.peopleAvailable}`);
        return;
      }
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="border-t border-white p-4 font-mono">
      <div className="flex flex-col gap-2 md:flex-row md:items-center">
        <label className="text-white/70 text-sm md:w-48">
          {getLabel()}
        </label>
